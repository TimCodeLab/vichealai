<template>
  <ion-page>
      ...
      <div class="role-chips-wrap">
        <div class="role-chips">
          ...
          >
            ...
            <span v-if="!role.isSystem" class="rchip-del" @click.stop="deleteRole(role.id)">×</span>
          </button>
        </div>
      </div>

      <div v-if="selectedRole" class="content-wrap">

        <!-- Role info card -->
        <div class="role-card">
          <div class="role-icon-big" :style="{ background: selectedRole.color + '22' }">{{ selectedRole.icon }}</div>
          <div class="role-meta">
            <div class="role-name">{{ selectedRole.name }}</div>
            <div class="role-desc">{{ selectedRole.description || t('roles.noDesc') }}</div>
            <div class="role-tag" :style="{ background: selectedRole.color + '22', color: selectedRole.color }">
              {{ selectedRole.isSystem ? t('roles.system') : t('roles.custom') }}
            </div>
          </div>
          <button v-if="!selectedRole.isSystem" class="edit-btn" @click="openEditRole(selectedRole)">✏️</button>
        </div>

        <!-- Permission matrix -->
        <div class="matrix-section">
          <div class="matrix-title">🛡 {{ t('roles.permissions') }}</div>
          <div class="matrix-table">
            <div class="matrix-head">
              <div class="mh-module">{{ t('roles.module') }}</div>
              <div v-for="action in actions" :key="action" class="mh-action">{{ t('roles.' + action) }}</div>
            </div>
            <div v-for="mod in modules" :key="mod.key" class="matrix-row">
              <div class="mr-module">
                <span class="mr-icon">{{ mod.icon }}</span>
                <span class="mr-name">{{ t('nav.' + mod.key) }}</span>
              </div>
              <div v-for="action in actions" :key="action" class="mr-cell">
                <button
                  class="perm-toggle"
        ...
        <button v-if="!selectedRole.isSystem" class="btn-save" @click="savePermissions">
    ...
    <!-- Role modal -->
    <ion-modal :is-open="showModal" @didDismiss="showModal = false" :breakpoints="[0,1]" :initial-breakpoint="1">
      <ion-content style="--background:white">
        <div class="modal-inner">
          <div class="modal-title">{{ editing ? '✏️ ' + t('roles.editRole') : '➕ ' + t('roles.newRole') }}</div>

          <div class="field-group">
            <label class="field-label">{{ t('roles.roleName') }} *</label>
            <input v-model="form.name" class="field-input" :placeholder="t('roles.roleNamePlaceholder')" />
          </div>
          <div class="field-group">
            <label class="field-label">{{ t('roles.roleDesc') }}</label>
            <input v-model="form.description" class="field-input" :placeholder="t('roles.roleDescPlaceholder')" />
          </div>
          <div class="field-group">
          ...
          <div class="modal-btns">
            <button class="mbtn-cancel" @click="showModal = false">{{ t('actions.cancel') }}</button>
            <button class="mbtn-save" @click="saveRole">{{ t('actions.save') }}</button>
          </div>
        </div>
      </ion-content>
    </ion-modal>
...
import { IonPage, IonHeader, IonToolbar, IonTitle, IonContent, IonButtons, IonBackButton, IonButton, IonModal } from '@ionic/vue'
