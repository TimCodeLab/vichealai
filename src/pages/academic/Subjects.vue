  ...
  <ion-page>
    ...
    <ion-content style="--background:#f5f7fa">
      <div class="pg-body anim-fade-up">
        ...
        <div class="pg-search">
          ...
          <input v-model="searchQuery" class="pg-search-input" :placeholder="t('subjects.search')" />
        ...
        <div class="grade-filter">
          ...
          >{{ g === 'all' ? t('subjects.allGrades') : g }}</button>
        </div>
        ...
        <div v-if="filtered.length === 0" class="pg-empty">
          ...
          <div class="empty-title">{{ t('subjects.notFound') }}</div>
          <div class="empty-sub">{{ t('subjects.notFoundSub') }}</div>
          <button class="pg-empty-btn" @click="openCreate">+ {{ t('subjects.add') }}</button>
        </div>

        <!-- Subject cards -->
        <div v-for="sub in filtered" :key="sub.id" class="sub-card">
          <div class="sub-icon" :style="{ background: subColor(sub.name) }">
            {{ sub.name?.charAt(0).toUpperCase() }}
          </div>
          <div class="sub-info">
            <div class="sub-name">{{ sub.name }}</div>
            <div class="sub-meta-row">
              <span v-if="sub.code" class="sub-code">{{ sub.code }}</span>
              <span v-if="sub.grade" class="sub-grade">Grade {{ sub.grade }}</span>
              <span v-if="sub.credits" class="sub-credits">{{ sub.credits }} cr</span>
            </div>
            <div v-if="sub.teacher" class="sub-teacher">👩‍🏫 {{ sub.teacher }}</div>
          </div>
          <div class="pg-actions">
            <button class="pg-btn pg-btn-edit" @click="editItem(sub)">
            ...
            <button class="pg-btn pg-btn-del" @click="deleteItem(sub.id)">
    ...
    <ion-modal :is-open="showModal" @did-dismiss="closeModal">
      <div class="mo-wrap">
        ...
        <div class="mo-body">
          <div class="mo-field"><label class="mo-label">{{ t('subjects.name') }} *</label><input v-model="form.name" class="mo-input" placeholder="e.g. Mathematics, គណិតវិទ្យា" /></div>
          <div class="mo-row2">
            <div class="mo-field"><label class="mo-label">{{ t('subjects.code') }}</label><input v-model="form.code" class="mo-input" placeholder="e.g. MATH01" /></div>
            <div class="mo-field"><label class="mo-label">{{ t('subjects.grade') }}</label>
              <select v-model="form.grade" class="mo-input">
                <option value="">--</option>
          ...
          <div class="mo-row2">
            <div class="mo-field"><label class="mo-label">{{ t('subjects.credits') }}</label><input v-model="form.credits" type="number" class="mo-input" placeholder="3" /></div>
            <div class="mo-field"><label class="mo-label">{{ t('subjects.teacher') }}</label><input v-model="form.teacher" class="mo-input" placeholder="Teacher name" /></div>
          </div>
          <div class="mo-btns">
